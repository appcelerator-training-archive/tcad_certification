...
String.toLocaleString({
	"en-US": {
		...
		"%l_ti_include": "Ti.include()",
		"%l_commonjs": "CommonJS require()",
		"%l_windows": ">Windows with URL properties",
		"%l_one_file": "One file per window is not recommended*",
		"%l_usage_demos": "Usage Demos",
		"%l_slidenote_structuring_js_code": "Ti.include() - code injected into current execution context<br> CommonJS – set up isolated scope and return only published properties and methods<br> All three JavaScript code inclusion methods are demonstrated in the Kitchen Sink.<br> One file per window is only recommended when a “clean slate” for execution is desired<br> Ti.include() Demo:<br> <ol> <li>Create new project</li> <li>Launch and run in simulator</li>				<li>Open app.js</li> <li>Use Ti.include statement to include file (Android: absolute paths / iOS: relative to current directory based on context)</li> <li>Create that include file—add log statement</li><li>Launch, show log statement in console</li><br>CommonJS Demo:<ol><li>In app.js, enter require(‘logger’)</li><li>Create logger.js file:exports.info = function(_str) { Ti.API.info(_str); }</li><li>In app.js, enter logger.info(‘logged with common js’);</li><li>Launch and show console</li></ul>",
		...
		"%l_functionality": "Platform-specific functionality is broken out into sub-namespaces where appropriate",
		"%l_slidenote_l_internationalization": "IProject/i18n/<i>lang</i>/strings.xml<br><i>lang</i> = language name per ISO 639-1 standard<br><i>component</i>refers to most any of the UI elements: buttons, views, etc.<br>Use titleid instead of title property<br>app.xml defines the internationalized version of your app’s name",
		"%l_qa": "Q&A",
